...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (empty($_POST['school_id'])) {
        ...
        $sql = "INSERT INTO addsection (school_id, section) VALUES ('$school_id', '$section')";
        if (mysqli_query($conn, $sql)) {
            echo "<script>alert('Section added successfully'); window.location.href=window.location.href;</script>";
        }
    }
}

// Fetching classes for the table
$classQuery = "SELECT * FROM addsection WHERE school_id = '$schoolId'";
    ...
    <div class="form-container">
        ...
        <form method="post">
            ...
            <div class="form-group">
                ...
                <input type="text" id="className" name="section" placeholder="Add Section (Capital latter)">
            </div>
            ...
            <tbody>
                ...
                if (mysqli_num_rows($classResult) > 0) {
                    while ($row = mysqli_fetch_assoc($classResult)) {
                        ...
                        <td><a href='delete_section.php?id={$row['id']}' style='color:red; text-decoration:none;'>Delete</a></td>
                      </tr>";
                        // $i++;
                    }
                } else {
                    echo "<tr><td colspan='2' style='text-align:center;'>No Section added yet.</td></tr>";
                }
                ?>
            </tbody>
        </table>
