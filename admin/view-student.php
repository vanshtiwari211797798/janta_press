                ...
                <tbody>
                    ...
                    if (mysqli_num_rows($data) > 0) {
                        while ($record = mysqli_fetch_assoc($data)) {
                            // Generate the QR code image dynamically for each student based on their ID
                            $qrCodeFile = 'phpqrcode/student_' . $record['id'] . '.png';  // Path where the QR code will be saved
                            QRcode::png($record['id'], $qrCodeFile); // Generate QR code and save it
                    ?>
                            <tr>
                                <td><?= $record['id'] ?></td>
                                <td><?= $record['school_id'] ?></td>
                                <td><img src="<?= $qrCodeFile ?>" height="80px" width="80px" alt="QR Code"></td>
